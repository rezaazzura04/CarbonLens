"""
CarbonLens — Organisation, User, Role, Session models.
...
class User(TypedDict):
    """
    Local session identity with role assignment.
    No password/login system exists in this build (portfolio /
    pre-commercial deployment) — this shape matches the actual runtime
    identity (see services.auth_service.DEMO_USER), not a credentialed
    account record.
    """
    username:      str          # e.g. "demo_user"
    display_name:  str
...
class Session(TypedDict):
    ...
    active_page:   str               # destination ID
