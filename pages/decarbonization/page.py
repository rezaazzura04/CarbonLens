"""
CarbonLens — Decarbonization Planner. Phase 5-A.
...
def render() -> None:
    ...
    if not org or not org.get("company_name"):
        ...
        empty_state("decarbonization", "No organisation configured",
...
def _s2_baseline(
    ...
    with c1:
        kpi_card("Baseline Total", f"{total_tco2e:.1f}",
                 ...
                 icon_name="database")
    with c2:
        kpi_card("Scope 1", f"{scope1_tco2e:.2f}", delta_label="tCO2e", icon_name="carbon")
    with c3:
        kpi_card("Scope 2", f"{scope2_tco2e:.2f}", delta_label="tCO2e", icon_name="database")
    with c4:
        kpi_card("Scope 3", f"{scope3_tco2e:.2f}", delta_label="tCO2e", icon_name="layers")
...
def _s3_target_definition(
    ...
    if target:
        ...
        with c1:
            kpi_card("Reduction Target",   f"{tgt_pct:.1f}%", badge="User-provided", badge_type="blue", icon_name="target")
        with c2:
            kpi_card("Target Emissions",   f"{tgt_t:.1f}", delta_label=f"tCO2e by {tgt_yr}", icon_name="clock")
        with c3:
            kpi_card("Required Reduction", f"{remaining:.1f}", delta_label="tCO2e to remove", icon_name="trending_down")

    if can_edit:
        with st.expander("Set Reduction Target", expanded=not bool(target)):
...
def _s5_scenario_comparison(
    ...
    try:
        ...
        colors = ["#8A968D"] + [
            "#037C38" if not r.get("above_target", True) else "#C65A5A"
            for r in results
        ]
        ...
        if target_t:
            fig.add_hline(
                y=target_t, line_dash="dot", line_color="#D89A3D",
    ...
    for r in results:
        ...
        color   = "#037C38" if not above else "#C65A5A"
        gap_lbl = r.get("gap_label", "")
        st.markdown(
            f'<div style="border-left:4px solid {color};padding:8px 12px;'
            f'background:#F5F7F2;border-radius:0 8px 8px 0;margin-bottom:6px;">'
            f'<span style="font-weight:{WEIGHT_BOLD};color:#17221C;">'
...
def _s6_action_plan(results: list, active_scenario: dict, target: dict) -> None:
    ...
    for lever in sorted(levers, key=lambda l: l.get("implementation_year", 2030)):
        ...
        color = "#3E7C8C" if atype == "user-provided" else "#8A968D"

        st.markdown(
            f'<div style="display:flex;gap:12px;padding:8px 0;'
            f'border-bottom:1px solid #DDE5DE;align-items:center;">'
            f'<span style="font-size:13px;font-weight:{WEIGHT_BOLD};color:{color};'
            f'min-width:48px;">{yr}</span>'
            f'<span style="font-size:{SIZE_SM};color:#17221C;">{name}</span>'
            f'<span style="font-size:{SIZE_XS};color:#64748B;">−{pct:.0f}%</span>'
            f'<span style="font-size:{SIZE_XS};background:{"#E3EEF0" if atype=="user-provided" else "#DDE5DE"};'
...
def _s7_assumption_transparency(
    ...
    provenance_panel(
        ...
        expanded         = False,
        methodology_note = (
            "CarbonLens Phase 5-A · Modelled estimates only · "
...
def _s8_quick_actions() -> None:
    ...
    with col1:
        if st.button("Executive Summary", **width_stretch_kwargs(), key="decarb_qa_exec"):
            state_svc.navigate_to("executive_summary"); st.rerun()
    with col2:
        if st.button("Carbon Accounting", **width_stretch_kwargs(), key="decarb_qa_carbon"):
            state_svc.navigate_to("carbon_accounting"); st.rerun()
    with col3:
        if st.button("Reporting", **width_stretch_kwargs(), key="decarb_qa_report"):
...
def _label_note(context: str, source: str) -> None:
    ...
    st.markdown(
        ...
        f'<span style="background:#DDE5DE;padding:1px 6px;border-radius:8px;">'
