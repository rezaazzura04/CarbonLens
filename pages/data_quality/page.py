"""
CarbonLens — Data Quality Workspace.
...
from components.theme.colors import (
    ...
    SUCCESS_LIGHT, WARNING_LIGHT, ERROR_LIGHT, BORDER, TEXT_SECONDARY, TEXT_MUTED,
)
...
def render() -> None:
    ...
    if not org or not org.get("company_name"):
        ...
        empty_state("data_quality", "No organisation configured",
...
def _s2_overall_quality(
    ...
    with c2:
        ...
        kpi_card("Completeness", f"{completeness:.0f}%",
                 ...
                 badge_type=comp_type, icon_name="percent")
    with c3:
        ...
        kpi_card("Consistency", f"{consistency:.0f}%",
                 ...
                 badge_type=cons_type, icon_name="check_circle")
    with c4:
        val_types = {"Pass": "green", "Warning": "yellow", "Fail": "red"}
        kpi_card("Validation", val_status,
                 badge=f"{val_score:.0f} pts",
                 badge_type=val_types.get(val_status, "neutral"), icon_name="search")
    with c5:
        ...
        for flag in group_flags:
            ...
            with col_info:
                st.markdown(
                    ...
                    f'<div style="font-size:{SIZE_SM};color:#66736B;margin-top:2px;">'
...
def _s5_validation_summary(
    ...
    with col_status:
        st.markdown(
            ...
            f'<div style="font-size:{SIZE_XS};font-weight:{WEIGHT_BOLD};'
            f'text-transform:uppercase;letter-spacing:{TRACKING_WIDE};'
            f'color:#8A968D;margin-bottom:4px;">Upload Validation</div>'
...
def _s6_completeness(
    ...
    with col_env:
        ...
        for field, present in env_fields.items():
            icon_name = "check" if present else "circle"
            color = SUCCESS if present else ERROR
            st.markdown(
                f'<div style="display:flex;justify-content:space-between;align-items:center;'
                f'padding:5px 0;border-bottom:1px solid {BORDER};">'
                f'<span style="font-size:{SIZE_SM};color:{TEXT_SECONDARY};">{field}</span>'
                f'<span style="display:flex;align-items:center;gap:4px;font-size:12px;'
                f'font-weight:{WEIGHT_BOLD};color:{color};">'
                f'{icon(icon_name, size=13, color=color)}'
                f'<span>{"Present" if present else "Missing"}</span></span></div>',
                unsafe_allow_html=True,
            )
        spacer(4)
    ...
    with col_sg:
        ...
        for key, label in sg_field_labels.items():
            ...
            st.markdown(
                f'<div style="display:flex;justify-content:space-between;align-items:center;'
                f'padding:5px 0;border-bottom:1px solid {BORDER};">'
                f'<span style="font-size:{SIZE_SM};color:{TEXT_SECONDARY};">{label}</span>'
                f'<span style="display:flex;align-items:center;">'
                f'{icon(icon_name, size=13, color=color)}</span></div>',
                unsafe_allow_html=True,
            )
        spacer(4)
...
def _s7_quality_trends(history: list, current_conf: float) -> None:
    ...
    try:
        ...
        marker_colors = [
            val_colors.get(h["validation"], "#8A968D") for h in history
        ]
        ...
        fig.add_trace(go.Scatter(
            ...
            mode  = "lines+markers",
            name  = "DQ Confidence %",
            line  = dict(color="#3E7C8C", width=2.5),
            marker= dict(size=8, color=marker_colors,
                         line=dict(color="#FFFFFF", width=1.5)),
        ))
        # Provisional floor line
        fig.add_hline(y=50, line_dash="dot", line_color="#D89A3D",
                      annotation_text="Provisional floor (50%)",
                      annotation_position="bottom right")

        fig.update_layout(
            title        = dict(text="Data Quality Confidence History",
                                font=dict(size=13, color="#17221C")),
            xaxis        = dict(showgrid=False, color="#8A968D", tickangle=-30),
            yaxis        = dict(range=[0, 105], title="DQ Confidence %",
                                gridcolor="#DDE5DE", color="#8A968D"),
            plot_bgcolor = "#FFFFFF",
...
def _s8_methodology(prov_rows: list, dq_conf: float, dq_prov: bool, summary: str) -> None:
    ...
    with col_panel:
        provenance_panel(
            ...
            key              = "dq_prov",
            expanded         = False,
            methodology_note = "CarbonLens Phase 2 · Four-part blended confidence model",
            show_download    = True,
        )
    ...
    with col1:
        if st.button("Executive Summary", **width_stretch_kwargs(), key="dq_qa_exec"):
            state_svc.navigate_to("executive_summary"); st.rerun()
    with col2:
        if st.button("ESG Analytics", **width_stretch_kwargs(), key="dq_qa_esg"):
            state_svc.navigate_to("esg_analytics"); st.rerun()
    with col3:
        if st.button("Governance", **width_stretch_kwargs(), key="dq_qa_gov"):
