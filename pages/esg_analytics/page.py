...
from components.theme.colors       import (
    ...
    SUCCESS, ERROR, BORDER, TEXT_SECONDARY, TEXT_MUTED,
)
...
def render() -> None:
    ...
    if not org or not org.get("company_name"):
        ...
        empty_state("esg", "No organisation configured",
    ...
    meth_ver = str(esg.get("methodology_version", "Phase4"))
...
def _handle_form_submission(org: dict, di_current: dict) -> dict:
    ...
    """
    with st.expander("Enter Social & Governance Indicators", expanded=not bool(di_current)):
        ...
        # RBAC (Fix F-06): editing S/G disclosure inputs is a data-mutation
        # action, gated the same as data upload — a viewer can see the form
        # but cannot submit changes.
        can_edit = state_svc.check_permission("can_upload")
        if not can_edit:
            st.caption("Your role does not have permission to update ESG indicators.")
        submitted = st.button(
            "Update ESG Score", type="primary", key="esg_submit", disabled=not can_edit,
        )
...
def _s2_overall_score(
    ...
    with col_pillars:
        spacer(24)
        pillar_icons = {"Environmental": "esg", "Social": "layers", "Governance": "edit"}
        for pillar, sc, color, bg in [
            ...
            st.markdown(
                ...
                f'margin-bottom:6px;position:relative;">'
                f'<div style="position:absolute;top:10px;right:12px;">'
                f'{icon(pillar_icons[pillar], size=15, color=color, stroke_width=1.75)}</div>'
                f'<div style="display:flex;justify-content:space-between;padding-right:20px;">'
...
def _s3_environmental(
    ...
    with col_chart:
        ...
        scope_bar_chart(
            labels     = sub_labels,
            values     = sub_values,
            colors     = ["#037C38","#037C38","#34D399","#6EE7B7"],
    ...
    with col_detail:
        ...
        for (sub, wt), val in zip(weights, sub_values):
            st.markdown(
                ...
                f'padding:4px 0;border-bottom:1px solid #DDE5DE;">'
                f'<span style="font-size:11px;color:#66736B;">{sub} <span '
                f'style="font-size:9px;color:#8A968D;">({wt})</span></span>'
...
def _s4_social(
    ...
    with col_chart:
        scope_bar_chart(
            labels = sub_labels,
            values = sub_values,
            colors = ["#3E7C8C","#818CF8","#A5B4FC","#C7D2FE"],
    ...
    with col_inputs:
        ...
        for name, val in disclosed_inputs.items():
            ...
            icon_name = "check" if disclosed else "circle"
            color = SUCCESS if disclosed else ERROR
            st.markdown(
                f'<div style="display:flex;justify-content:space-between;align-items:center;'
                f'padding:5px 0;border-bottom:1px solid {BORDER};">'
                f'<span style="font-size:11px;color:{TEXT_SECONDARY};">{name}</span>'
                f'<span style="display:flex;align-items:center;gap:4px;font-size:12px;font-weight:700;color:{color};">'
                f'{icon(icon_name, size=12, color=color)}<span>{val if disclosed else "Not disclosed"}</span></span></div>',
                unsafe_allow_html=True,
            )
        spacer(4)
        st.caption(
...
def _s5_governance(
    ...
    with col_chart:
        scope_bar_chart(
            labels = sub_labels,
            values = sub_values,
            colors = ["#D89A3D","#FB923C","#FDBA74","#FED7AA","#FBF1DF"],
    ...
    with col_inputs:
        ...
        for name, val in g_items.items():
            ...
            st.markdown(
                ...
                f'<span style="font-size:11px;color:{TEXT_SECONDARY};">{name}</span>'
                f'<span style="display:flex;align-items:center;gap:4px;font-size:12px;font-weight:700;color:{color};">'
                f'{icon(icon_name, size=12, color=color)}<span>{disp}</span></span></div>',
                unsafe_allow_html=True,
            )
        spacer(4)
        st.caption(
...
def _s6_confidence_methodology(
    ...
    provenance_panel(
        ...
        key              = "esg_prov",
        expanded         = False,
        methodology_note = "GRI 2021 aligned · E=40%, S=30%, G=30% · CarbonLens",
        show_download    = True,
    )
    spacer(8)
...
def _s7_benchmark(
    ...
    with col_carbon:
        ...
        st.markdown(
            ...
            f'<div style="font-size:9px;font-weight:700;text-transform:uppercase;'
            f'letter-spacing:0.8px;color:#8A968D;">Carbon vs {sector} Benchmark</div>'
            ...
            f'<div style="font-size:9px;color:#8A968D;margin-top:8px;">'
    ...
    with col_esg:
        esg_color = "#037C38" if esg_score >= 75 else "#D89A3D" if esg_score >= 50 else "#C65A5A"
        esg_label = (
        ...
        st.markdown(
            ...
            f'border-radius:12px;padding:18px 16px;">'
            f'<div style="font-size:9px;font-weight:700;text-transform:uppercase;'
            f'letter-spacing:0.8px;color:#8A968D;">ESG Score Interpretation</div>'
...
def _s8_recommendations(recs: list) -> None:
    ...
    if not recs:
        st.markdown(
            ...
            f'{icon("check_circle", size=18, color=TEXT_MUTED)}'
            f'<span>No high-priority recommendations — good work!</span></div>',
            unsafe_allow_html=True,
        )
        return
    ...
    pillar_colors = {
        ...
        "G": GOV_COLOR, "DQ": "#3E7C8C", "All": "#3E7C8C",
    }
    pillar_labels = {
    ...
    for i, rec in enumerate(recs):
        ...
        st.markdown(
            ...
            f'background:#F5F7F2;padding:12px 16px;margin-bottom:8px;">'
            ...
            f'<div style="font-size:13px;font-weight:700;color:#17221C;">'
            f'{rec.get("title","")}</div>'
    ...
    with col1:
        if st.button("Executive Summary", **width_stretch_kwargs(), key="esg_qa_exec"):
            state_svc.navigate_to("executive_summary"); st.rerun()
    with col2:
        if st.button("Carbon Accounting", **width_stretch_kwargs(), key="esg_qa_carbon"):
            state_svc.navigate_to("carbon_accounting"); st.rerun()
    with col3:
        if st.button("Generate Report", **width_stretch_kwargs(), key="esg_qa_report"):
    ...
    return [
        {
            ...
            "value":   "40%",
            "source":  "CarbonLens Methodology Library",
            "formula": "E × 0.40",
