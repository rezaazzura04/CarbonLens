"""
CarbonLens — Governance Workspace.
...
from components.theme.colors import (
    ENV_COLOR, SOC_COLOR, GOV_COLOR, BRAND_ACCENT, BORDER, TEXT_MUTED,
...
def _s3_methodology_library(entries: list) -> None:
    ...
    info_banner(
        ...
        "internal methodology — they are not published by, endorsed by, or "
        "derived from GRI. Where a GRI code is shown, it identifies the "
        "related disclosure *topic* only (e.g. GRI 305 = Emissions), not the "
        "source of the specific weight or formula.",
        "info",
    )
    spacer(4)
    ...
    cat_colors = {
        ...
        "ESG Grade Bands":              "#0F3D2E",
        "Confidence Model":             "#D89A3D",
        "Data Quality Model":           "#3E7C8C",
        "GHG Inventory":                "#037C38",
        "Sector Benchmarking":          "#C65A5A",
    }
    ...
    for cat, cat_entries in categories.items():
        ...
        with st.expander(f"{cat} ({len(cat_entries)} entries)", expanded=False):
            ...
            st.markdown(
                ...
                f'gap:4px;padding:4px 0;border-bottom:1.5px solid #DDE5DE;'
                f'font-size:{SIZE_XS};font-weight:{WEIGHT_BOLD};text-transform:uppercase;'
                f'letter-spacing:{TRACKING_WIDE};color:#8A968D;">'
                f'<div>Name</div><div>Value</div><div>Formula</div><div>Source · Related GRI Topic</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
            for i, entry in enumerate(cat_entries):
                bg = "#F5F7F2" if i % 2 == 0 else "#FFFFFF"
                ...
                st.markdown(
                    ...
                    f'border-bottom:1px solid #DDE5DE;font-size:{SIZE_BASE};">'
                    f'<div style="font-weight:{WEIGHT_BOLD};color:#17221C;">'
                    ...
                    f'<div style="color:#8A968D;font-size:{SIZE_SM};">{src_str}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
                if entry.get("rationale"):
                    st.markdown(
                        f'<div style="font-size:{SIZE_XS};color:#8A968D;'
                        f'padding:2px 4px 6px 4px;background:{bg};">'
                        f'↳ {entry["rationale"]}</div>',
                        unsafe_allow_html=True,
                    )

    st.caption(f"{len(entries)} methodology entries · CarbonLens · GRI 2021 aligned")
    spacer(8)
    ...
    for cat, cat_entries in categories.items():
        ...
        with st.expander(f"{cat} ({len(cat_entries)} factors)", expanded=False):
            ...
            for i, ef in enumerate(cat_entries):
                ...
                h3_note    = (
                    f'<span style="font-size:{SIZE_XS};color:#D89A3D;"> ↑ was {co2_only}</span>'
                ...
                st.markdown(
                    ...
                    f'<div style="color:#64748B;font-family:monospace;font-size:{SIZE_SM};">'
                    f'{ef.get("unit","")}</div>'
                    f'<div style="color:#8A968D;font-size:{SIZE_SM};">{source_str}</div>'
                    f'<div style="color:#8A968D;font-size:{SIZE_SM};">'
...
def _s5_methodology_transparency() -> None:
    ...
    provenance_panel(
        ...
        expanded=False, methodology_note="CarbonLens Phase 2 · Four-part blended model",
    )
        ...
        if not report_events:
            ...
            for ev in report_events[:5]:
                ...
                st.markdown(
                    ...
                    f'font-size:{SIZE_SM};display:flex;align-items:center;gap:6px;">'
                    f'<span style="color:{TEXT_MUTED};font-family:monospace;">{ts}</span> '
                    f'{icon(icon_name, size=13, color=BRAND_ACCENT)}<span>{summ}</span></div>',
                    unsafe_allow_html=True,
                )
...
def _s7_governance_metrics(metrics: dict) -> None:
    ...
    with c4:
        kpi_card("Recomputations",
                 ...
                 badge_type="neutral", icon_name="refresh")

    spacer(8)

    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("Executive Summary", **width_stretch_kwargs(), key="gov_qa_exec"):
            state_svc.navigate_to("executive_summary"); st.rerun()
    with col2:
        if st.button("Data Quality", **width_stretch_kwargs(), key="gov_qa_dq"):
            state_svc.navigate_to("data_quality"); st.rerun()
    with col3:
        if st.button("Reporting", **width_stretch_kwargs(), key="gov_qa_report"):
