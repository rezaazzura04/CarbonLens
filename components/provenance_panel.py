"""
CarbonLens — Provenance panel component.
    ...
    with st.expander(f"How {title} was calculated", expanded=expanded):
        ...
        for i, row in enumerate(rows):
            ...
            if row.get("note"):
                ...
                notes += f'{sep}<span style="color:#D89A3D;font-style:italic;">{row["note"]}</span>'

            st.markdown(
                f'<div style="display:grid;grid-template-columns:2fr 1.5fr 2fr 2fr;'
                f'gap:4px;padding:6px 4px;background:{bg};'
                f'border-bottom:1px solid #DDE5DE;font-size:{SIZE_BASE};color:{TEXT_PRIMARY};">'
        ...
        if methodology_note:
            st.markdown(
                ...
                f'margin-top:8px;padding-top:6px;border-top:1px solid #DDE5DE;">'
...
def methodology_footnote(version: str = "Phase4", gri: str = "GRI 2021") -> None:
