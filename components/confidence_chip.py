"""
CarbonLens — Confidence chip component.
...
def confidence_chip(
    ...
    if compact:
        ...
        st.markdown(
            ...
            f'{label}{(" — " + score_s) if score_s else ""}</span>'
            f'<span style="font-size:{SIZE_SM};color:#8A968D;">{explanation}</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
...
def provisional_badge(label: str = "Provisional") -> None:
    ...
    st.markdown(
        f'<span style="background:#FBF1DF;color:#92400E;border:1px solid #FDE68A;'
...
def quality_band(dq: dict) -> None:
    ...
    def _sc(v: float) -> str:
        if v >= 80: return "#037C38"
        if v >= 60: return "#D89A3D"
        return "#C65A5A"

    val_col = {"Pass": "#037C38", "Warning": "#D89A3D", "Fail": "#C65A5A"}.get(
        val_status, "#C65A5A"
    )
    ...
    cards = [
        ("Completeness",  f"{completeness:.0f}%", _sc(completeness), "Env + S/G coverage",     "#FFFFFF", "#DDE5DE"),
        ("Consistency",   f"{consistency:.0f}%",  _sc(consistency),  "Rows passing checks",    "#FFFFFF", "#DDE5DE"),
        ("Validation",    val_status,              val_col,           "Upload validation",       "#FFFFFF", "#DDE5DE"),
    ...
    for col, (title, value, color, subtitle, card_bg, card_brd) in zip(cols, cards):
        with col:
            st.markdown(
                ...
                f'color:#8A968D;margin-bottom:4px;">{title}</div>'
