...
def test_methodology_library_length():
    ...
    assert len(lib) == 30, f"Expected 30, got {len(lib)}"  # +1: Sector Benchmarking provenance entry (Fix C5)
...
def test_components_have_no_service_calls():
    ...
    Exceptions:
      - sidebar_nav.py may import auth_service for display of the current
        user (presentation concern, not business logic).
      - sidebar_nav.py may import state_service ONLY for its thin
        navigation-state passthroughs (get_active_page/set_active_page/
        get_active_slot/set_active_slot) — added in Phase 5-B remediation
        so components never import repository.session_repo directly
        (previously sidebar_nav.py imported repository.session_repo
        directly, which was itself an architecture violation).
    No component may import repository.* directly, under any circumstance —
    that invariant is checked unconditionally below.
    """
    ...
    state_service_exception_files = {os.path.join('components', 'sidebar_nav.py')}
        ...
        for f in sorted(files):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    ...
                    if mod == 'services.state_service' and path not in state_service_exception_files:
                        violations.append(f"{path}:{node.lineno}: services.state_service (no exception for this file)")
                    # No component may import repository.* directly — this is the
                    # invariant the sidebar_nav.py fix exists to uphold.
                    if mod == 'repository' or mod.startswith('repository.'):
                        violations.append(f"{path}:{node.lineno}: {mod} (components must never import repository directly)")
