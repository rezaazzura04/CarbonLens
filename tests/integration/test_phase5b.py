...
def test_T1_app_has_no_login_gate():
    """T1 (revised): app.py has no login gate at all — always inits demo mode."""
    ...
    assert 'st.stop()' not in main_body, "st.stop() in main() — would block entry"
    assert 'CARBONLENS_AUTH_REQUIRED' not in main_body
    assert '_render_login' not in main_src
