...
def test_DEMO_01_fresh_launch_inits_demo(monkeypatch):
    """T-DEMO-01: fresh launch (login-free) → Demo Mode auto-init."""
    demo_called = []
...
def test_DEMO_05_all_destinations_importable():
    """T-DEMO-05: All 7 destinations must import without error."""
    import importlib
...
def test_DEMO_08_sidebar_has_demo_banner():
    """
    T-DEMO-08 (updated for Final IA Polish): the large Demo Mode banner/card
    must NOT live in the sidebar above navigation — Demo Mode status now
    appears as a compact badge in the main page header instead, so the
    sidebar stays stable and calm regardless of demo/real state. Verifies
    the new location, not the old one.
    """
    sidebar_src = open('components/sidebar_nav.py').read()
    assert '_render_demo_banner' not in sidebar_src, (
        "Demo Mode banner must not be a large sidebar card — see IA polish brief §9"
    )
    ui_src = open('components/ui.py').read()
    assert 'is_demo_mode' in ui_src and 'DEMO MODE' in ui_src, (
        "Demo Mode indicator must be driven by page_header() in components/ui.py"
    )


def test_DEMO_08_demo_banner_has_disclaimer():
    """Demo Mode's compact page-header badge must still clearly say DEMO MODE."""
    ui_src = open('components/ui.py').read()
...
# ── T-DEMO-13: No login gate — app always opens login-free (portfolio build) ──

def test_DEMO_13_no_login_gate_in_app():
    """
    T-DEMO-13 (revised): this build is login-free by design (portfolio/
    pre-commercial mode) — there is no login/password system anywhere in
    the codebase (see auth_service.py), and app.py must NOT contain an
    auth-required login gate. RBAC (has_permission/check_permission)
    remains as reusable infrastructure, gated behind the single local
    identity instead of a login screen.
    """
    src = open('app.py').read()
    assert 'auth_required' not in src
    assert '_render_login' not in src
    assert 'CARBONLENS_AUTH_REQUIRED' not in src
    # The login-free identity bootstrap must still run unconditionally.
    assert 'if not is_authenticated()' in src
    assert 'init_demo_mode()' in src
...
def test_DEMO_15_audit_event_types_unchanged():
    ...
    assert len(APPROVED_EVENT_TYPES) == 17  # +1: gis_query_run (Phase 5-C)
...
def _minimal_state():
    return {
        ...
        "esg":{"score":68.5,"grade":"B","label":"Satisfactory",
               ...
               "disclosure_summary":"","methodology_version":"Phase4",
