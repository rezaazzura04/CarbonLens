...
# ── T39–T40: No login gate (portfolio/pre-commercial build) ───────────────────

def test_T39_login_free_boot_inits_demo(monkeypatch):
    """T39 (revised): app boots login-free and always initializes demo identity."""
    ...
    assert "CARBONLENS_AUTH_REQUIRED" not in app_src

def test_T40_no_login_gate_in_app():
    """T40 (revised): app.py must not reference _render_login or an auth gate."""
...
def test_T46_phase4():
    ...
    assert len(get_methodology_library()) == 30  # +1: Sector Benchmarking provenance entry (Fix C5)
