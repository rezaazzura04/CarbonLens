"""
CarbonLens — End-to-end workflow integration tests (Sprint 11).

Covers five workflow scenarios:
  1. First-time user (local identity auto-init → onboarding → upload → compute)
...
class TestFirstTimeUserWorkflow:
    """
    Validates the full first-time user journey: local identity auto-init
    (no login screen, see services/auth_service.py) → org setup → upload →
    compute.
    """
