...
def test_scope1_nan_raises():
    """
    Regression test: `value < 0` alone does not catch NaN (NaN comparisons
    are always False in Python), so NaN previously passed validation
    silently and propagated into the total instead of being rejected.
    """
    from calculations.ghg import calculate_scope1
    with pytest.raises(ValueError, match="diesel_liters"):
        calculate_scope1(diesel_liters=float("nan"))


def test_scope1_infinite_raises():
    from calculations.ghg import calculate_scope1
    with pytest.raises(ValueError, match="diesel_liters"):
        calculate_scope1(diesel_liters=float("inf"))
