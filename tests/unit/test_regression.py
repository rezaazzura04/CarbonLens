"""
CarbonLens — Phase 0 regression tests.
...
def test_approved_event_types_count():
    ...
    # +1: gis_query_run (Phase 5-C)
    assert len(APPROVED_EVENT_TYPES) == 17, \
        f"Expected 17 audit event types, found {len(APPROVED_EVENT_TYPES)}"
...
def test_approved_destinations():
    """Navigation: exactly 7 destinations must be defined."""
    from config.constants import APPROVED_DESTINATIONS
    assert len(APPROVED_DESTINATIONS) == 7, \
        f"Expected 7 destinations, found {len(APPROVED_DESTINATIONS)}"
