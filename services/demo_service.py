"""
CarbonLens — Demo Service.
Single canonical demo initialization path.

Provides the TWO-MODE model (no login gate — see app.py):
  A. DEMO MODE    — demo_mode_enabled=True (default on first launch)
  B. REAL MODE    — demo_mode_enabled=False (after "Set Up My Organisation")
...
def enable_demo_mode() -> None:
    ...
    try:
        ...
        # Critical state transition: if this silently fails, the app has no
        # mode set at all on first launch (Phase 5-B Fix 9C).
        log.error(f"enable_demo_mode failed — app may start with no active mode: {exc}")
