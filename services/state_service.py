...
def get_computed_state(
    ...
    """
    from calculations.utilities import hash_dataframe, hash_inputs, hash_dict
    ...
    # Build the canonical input fingerprint from EVERY input that can
    # change the computed result — not just the dataset (Phase 6 fix; see
    # calculations.utilities.hash_inputs docstring for why disclosure/scope
    # hashes are included).
    df_hash          = hash_dataframe(df) if df is not None else ""
    disclosure_hash  = hash_dict(disclosure_inputs)
    scope_hash       = hash_dict(scope_inputs)
    input_hash = hash_inputs(org_id, period, df_hash, disclosure_hash, scope_hash)
...
def get_methodology_library() -> list:
    """
    Return the complete CarbonLens methodology library.
    ...
    entries = [
        ...
        {"entry_id":"env-pillar-weight", "category":"ESG Composite Weights",
         ...
         "source":"CarbonLens Methodology", "introduced_in":"Phase 0 C2",
        ...
        {"entry_id":"env-carbon-weight", "category":"Environmental Sub-indicators",
         ...
         "source":"CarbonLens Methodology", "introduced_in":"Phase 0 C1",
        ...
        {"entry_id":"esg-provisional-floor", "category":"Confidence Model",
         ...
         "gri_reference":"",
         "source":"CarbonLens Methodology", "introduced_in":"Phase 0 C3",
        ...
        {"entry_id":"dq-completeness-weight", "category":"Data Quality Model",
         ...
         "gri_reference":"",
         "source":"CarbonLens Phase 2", "introduced_in":"Phase 2",
        ...
        # ── Sector benchmarking ─────────────────────────────────────────────────
        {"entry_id":"sector-benchmark-provenance", "category":"Sector Benchmarking",
         "name":"Carbon intensity benchmarks", "value":"See Carbon Accounting page",
         "formula":"kg CO2e / m² / year, by sector",
         "gri_reference":"",
         "source": _benchmark_provenance_text(),
         "introduced_in":"Phase 0 C5",
         "rationale":"Illustrative sector estimates — independent validation required "
                     "before use in formal regulatory or GRI submissions."},
    ]
    return entries


def _benchmark_provenance_text() -> str:
    """Thin wrapper so get_methodology_library() surfaces the SAME provenance
    text used by calculations.benchmarking.get_benchmark_provenance(), rather
    than duplicating it — closes the Phase 0 C5 gap where this text existed
    in code but was never actually displayed anywhere in the app."""
    from calculations.benchmarking import get_benchmark_provenance
    return get_benchmark_provenance()
...
def get_emission_factor_library() -> list:
    ...
    for cat_key, cat_data in S3EF.items():
        entries.append({
            ...
            "last_reviewed": "Phase 0 H3",
            # Surfaces config.settings.SCOPE3_EMISSION_FACTORS[cat]["note"]
            # in the Governance → Emission Factor Library expander via the
            # same "notes" mechanism already used for Scope 1/2 (see the
            # Phase 0 H3 note above) — no second provenance system.
            "notes": cat_data.get("note", ""),
        })
...
def complete_onboarding(
    ...
    org_data   : Organisation dict assembled from the onboarding form.
                 Must NOT already contain a name-derived org_id — see Fix F-02
                 below. Callers that legitimately need a stable pre-existing
                 org_id (e.g. tests, migrations) may still set it explicitly
    ...
    slot       : Organisation slot index (0–4). Callers MUST pass the
                 caller's actual active slot (state_service.get_active_slot())
                 rather than relying on the default — see Fix F-01. The
                 default of 0 exists only for backward compatibility with
                 callers that intentionally target slot 0 (e.g. tests).

    Returns the persisted Organisation dict, with an added "_persisted" bool
    flag (Fix F-08) indicating whether disk persistence actually succeeded —
    the in-memory session state is always updated regardless, but a caller
    should not tell the user "Setup Complete" if "_persisted" is False.
    """
    import datetime, uuid

    # ── Fix F-02: org_id is assigned HERE, in the service layer — never in
    # the page layer, and never derived from company_name. If the slot
    # already holds an organisation with an org_id, that identity is
    # preserved (stable across re-onboarding the same slot). Otherwise a
    # fresh, collision-safe UUID4 is assigned. A caller that explicitly
    # supplies its own org_id (tests, migrations) is respected as-is.
    if not org_data.get("org_id"):
        existing_org_id = None
        try:
            from repository.session_repo import get_organisation as _get_existing
            _existing = _get_existing(slot)
            if _existing:
                existing_org_id = _existing.get("org_id")
        except Exception:
            existing_org_id = None
        org_data["org_id"] = existing_org_id or str(uuid.uuid4())
    ...
    # ── Fix F-08: track whether persistence genuinely succeeded so the page
    # never claims "Setup Complete" when nothing durable was saved.
    persisted = True
    ...
    try:
        ...
        disk_ok = save_organisation(org_data, slot)
        if disk_ok is False:
            persisted = False
            log.warning(
                f"complete_onboarding: disk persistence FAILED for slot {slot} "
                f"({org_data.get('company_name','')}). Session state was updated "
                f"so the current session keeps working, but the organisation "
                f"will not survive an app restart."
            )

        mark_onboarding_complete(slot)
        if df is not None:
            set_uploaded_df(df, slot=slot)
...
def get_current_username() -> str:
    ...
    # Intentionally silent: purely cosmetic display fallback, never a
    # correctness or persistence concern (Phase 5-B Fix 9C: reviewed, kept silent).
    try:
...
def get_active_page() -> str:
    """Return the current destination ID. Used by app.py routing."""
    try:
...
def set_active_page(page: str) -> None:
    """
    Set the current destination ID directly (no APPROVED_DESTINATIONS
    validation — components that already know the id is valid, e.g. the
    org-switcher's demo-exit shortcut, use this; page-level navigation
    should prefer navigate_to() above, which does validate).
    Exists so components never need to import repository.session_repo directly.
    """
    from repository.session_repo import set_active_page as _set
    _set(page)


def get_cached_export(org_id: str, version: int, fmt: str, extra_fingerprint: str = ""):
    """
    Return a cached (bytes, filename) export tuple, or None. Thin wrapper
    so pages never import calculations.utilities/state.cache directly
    (Phase 6 — deferred/cached Excel & PDF export).
    """
    from calculations.utilities import hash_strings
    from state import cache as Cache
    export_hash = hash_strings(org_id, str(version), fmt, extra_fingerprint)
    return Cache.get_export(export_hash)


def cache_export(org_id: str, version: int, fmt: str, data, filename: str, extra_fingerprint: str = "") -> None:
    """Store a generated export's bytes under its canonical fingerprint."""
    from calculations.utilities import hash_strings
    from state import cache as Cache
    export_hash = hash_strings(org_id, str(version), fmt, extra_fingerprint)
    Cache.put_export(export_hash, (data, filename))


def get_active_slot() -> int:
    """
    Return the active organisation slot index (0–4).
    Single canonical entry point for slot-aware pages/components — see
    Phase 5-B Fix F-01. Callers that need to persist data for "the
    organisation the user is currently looking at" should read this once
    and pass it through explicitly, rather than letting a downstream
    function assume slot 0.
    """
    from repository.session_repo import get_active_slot as _get
    return _get()


def set_active_slot(slot: int) -> None:
    """Set the active organisation slot index (0–4)."""
    from repository.session_repo import set_active_slot as _set
    _set(slot)


# ── Spatial / Land-Use Context (Phase 5-C, Experimental) ──────────────────────

def get_spatial_context() -> Optional[dict]:
    """Return the last spatial query result for the active slot, or None."""
    from repository.session_repo import get_spatial_context as _get
    return _get()


def is_spatial_panel_open() -> bool:
    """Return True if the user has explicitly opened the Spatial Context tab
    for the active organisation slot this session."""
    from repository.session_repo import is_spatial_panel_open as _get
    return _get()


def run_spatial_query(lat: float, lon: float, radius_km: float, province: str) -> dict:
    """
    Run the Experimental spatial query for the active organisation and
    persist the result to session for the active slot. Delegates all actual
    orchestration (real-vs-synthetic decision, calculations, audit event)
    to services.gis_service — this wrapper's only job is wiring the result
    into slot-scoped session state, matching every other state_service
    write-path in this module.
    """
    from repository.session_repo import set_spatial_context, set_spatial_panel_open
    from services.gis_service import run_spatial_query as _run
    org = get_active_organisation()
    result = _run(
        lat=lat, lon=lon, radius_km=radius_km, province=province,
        org_id=org.get("org_id", "") if org else "",
        company_name=org.get("company_name", "") if org else "",
    )
    set_spatial_context(result)
    set_spatial_panel_open(True)
    return result


# ── Onboarding wizard state (Fix: architecture — no direct st.session_state
# access from pages/onboarding/page.py) ────────────────────────────────────────
#
# The onboarding wizard is a single, non-concurrent, multi-step form. Its
# scratch state (partially-filled profile fields, the pending upload, which
# step the wizard is on) lives in session_repo's global (non-slot) namespace
# for the duration of the wizard, and is cleared once setup completes or the
# user leaves. Pages talk to this state only through the functions below —
# never through st.session_state directly.

def get_onboarding_field(field: str, default=None):
    """Return a single onboarding-wizard scratch field (e.g. 'company')."""
    from repository.session_repo import get_onboarding_field as _get
    return _get(field, default)

def set_onboarding_field(field: str, value) -> None:
    """Set a single onboarding-wizard scratch field."""
    from repository.session_repo import set_onboarding_field as _set
    _set(field, value)

def pop_onboarding_field(field: str, default=None):
    """Pop (read + remove) a single onboarding-wizard scratch field."""
    from repository.session_repo import pop_onboarding_field as _pop
    return _pop(field, default)

def clear_onboarding_fields() -> None:
    """Clear all onboarding-wizard scratch fields (called after setup completes)."""
    from repository.session_repo import clear_onboarding_fields as _clear
    _clear()

def get_onboarding_step() -> int:
    """Return the current onboarding wizard step (1–3)."""
    from repository.session_repo import get_onboarding_step as _get
    return _get()

def set_onboarding_step(step: int) -> None:
    """Set the current onboarding wizard step (1–3)."""
    from repository.session_repo import set_onboarding_step as _set
    _set(step)
...
def get_forecast_validation() -> dict:
    ...
    Phase 6: cached by the SAME canonical input-hash principle as
    ComputedState — same (df, dq_status) → same cached result; a changed
    dataset or DQ status invalidates automatically because the hash
    changes, never because of a timer. Forecast validation runs a
    chronological holdout + naive-baseline comparison, which is real work
    to repeat on every page render that displays it (Executive Summary,
    Carbon Accounting, and report_service.build_report_context all call
    this) — caching it here benefits every one of those call sites
    without any of them needing to know caching exists.
    """
    try:
        from repository.session_repo import get_uploaded_df
        df = get_uploaded_df()
    ...
    try:
        ...
        # dq_status silently staying None changes forecast-gate behaviour
        # (treated as "no DQ status available" rather than an explicit
        # Pass/Warning/Fail), so this is worth a trace-level note even
        # though it's non-fatal (Phase 5-B Fix 9C).
        log.debug(f"get_forecast_validation: could not read DQ status, treating as unknown: {exc}")

    from calculations.utilities import hash_dataframe, hash_strings
    from state import cache as Cache

    df_hash = hash_dataframe(df) if df is not None else ""
    fcast_hash = hash_strings("forecast", str(dq_status), df_hash)

    cached = Cache.get_forecast(fcast_hash)
    if cached is not None:
        log.debug(f"Forecast cache HIT (hash={fcast_hash[:12]}...)")
        return cached

    from calculations.forecasting import forecast_with_validation
    result = forecast_with_validation(df, dq_validation_status=dq_status)
    Cache.put_forecast(fcast_hash, result)
    return result
