"""
CarbonLens — ComputedState assembly and invariant enforcement.
...
def assemble(
    ...
    # ── Input hash ──────────────────────────────────────────────────────────
    # Phase 6 fix: this previously always self-derived a hash from only
    # (org_id, period, carbon['scope_source']) — scope_source is a category
    # label like "csv"/"manual", NOT the actual data, so two uploads with
    # different emission VALUES but the same source type produced an
    # IDENTICAL input_hash here — a duplicate, less-accurate fingerprint
    # implementation living alongside the correct one in
    # services.state_service.get_computed_state() (which already hashes the
    # real dataframe content). Callers that know the real input fingerprint
    # (state_service) now pass it in explicitly; assemble() only falls back
    # to the old category-based derivation for callers that don't (kept for
    # backward compatibility with existing direct assemble() callers/tests
    # that don't exercise input_hash correctness specifically).
...
def _derive_input_hash(org_id: str, period: str, carbon: dict) -> str:
    """
    FALLBACK ONLY — derives a coarse hash from org_id/period/scope_source
    category, not actual dataset content. A caller that has the real
    dataset content available should compute a proper fingerprint (see
    calculations.utilities.hash_inputs) and pass it into assemble() as
    input_hash= explicitly; this function only exists so assemble() still
    returns a well-formed (if imprecise) ComputedState for callers that
    don't have that available (e.g. direct unit tests of assemble() itself).
    """
    import hashlib
