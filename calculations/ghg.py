"""
CarbonLens — GHG inventory calculations (Scope 1, 2, 3).
...
def _validate_non_negative(name: str, value: float) -> None:
    ...
    Bug fix: `value < 0` alone does not catch NaN or +/-inf — NaN
    comparisons are always False in Python, so `float('nan') < 0` is
    False and previously passed through silently, propagating NaN into
    the total and into JSON/CSV/PDF exports instead of being rejected
    at the same boundary as a negative number. Both are exactly as
    "not physically meaningful" as a negative quantity.
    """
    import math
    if isinstance(value, (int, float)) and not math.isfinite(value):
        raise ValueError(
            f"{name} must be a finite number, got {value}. "
            "NaN and infinite emission quantities are not physically meaningful."
        )
    if value < 0:
        raise ValueError(
